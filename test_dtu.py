...
def save_scene_depth(testlist):
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            view_ids_list = tensor2numpy(view_ids)
            ...
            for filename, cam, img, depth_est, photometric_confidence in zip(filenames, cams, imgs, \
                ...
                cor_weight_filename = os.path.join(args.outdir, filename.format('cor_weight', '.png'))
                # cor_weight_filename = os.path.join(args.outdir, filename.format('cor_weight_gt', '.png'))
                cam_filename = os.path.join(args.outdir, filename.format('cams', '_cam.txt'))
                img_filename = os.path.join(args.outdir, filename.format('images', '.jpg'))
                ply_filename = os.path.join(args.outdir, filename.format('ply_local', '.ply'))
                os.makedirs(depth_filename.rsplit('/', 1)[0], exist_ok=True)
                os.makedirs(confidence_filename.rsplit('/', 1)[0], exist_ok=True)
                os.makedirs(cor_weight_filename.rsplit('/', 1)[0], exist_ok=True)
...
if __name__ == '__main__':
    ...
    save_depth(testlist)
    print(dist_base, rel_diff_base, diff_base, diff_base_3d)

    if args.dataset.startswith('general'):
        # step2. filter saved depth maps with photometric confidence maps and geometric constraints
        print("dypcd_filter")
        dypcd_filter(testlist)
