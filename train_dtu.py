...
# main function
def train(model, model_loss, optimizer, train_sampler, TrainImgLoader, TestImgLoader, start_epoch, args):
    ...
    for epoch_idx in range(start_epoch, args.epochs):
        ...
        train_sampler.set_epoch(epoch_idx)  # Shuffle each epoch
        TrainImgLoader.dataset.reset_dataset(train_sampler)
...
def train_sample(model, model_loss, optimizer, sample, args):
    ...
    loss, stage_ce_loss, range_err_ratio, _ = model_loss(
...
if __name__ == '__main__':
    ...
    # model, optimizer
    model = EIA(arch_mode=args.arch_mode, reg_net=args.reg_mode, num_stage=4,
    ...
    if args.dataset.startswith('dtu'):
        train_dataset = MVSDataset(args.trainpath, args.trainlist, "train", 5, args.interval_scale, batch_size=args.batch_size, rt=args.rt,  use_raw_train=args.use_raw_train)
    ...
    train_sampler = torch.utils.data.DistributedSampler(train_dataset, num_replicas=1,rank=0)
    if is_distributed:

        test_sampler = torch.utils.data.DistributedSampler(test_dataset, num_replicas=dist.get_world_size(),
                                                           rank=0)

        TrainImgLoader = DataLoader(train_dataset, args.batch_size, sampler=train_sampler, num_workers=24,
                                    drop_last=True,
        ...
        TrainImgLoader = DataLoader(train_dataset, args.batch_size, shuffle=False, num_workers=24,
    ...
    if args.mode == "train":
        train(model, model_loss, optimizer, train_sampler, TrainImgLoader, TestImgLoader, start_epoch, args)
    elif args.mode == "test":
